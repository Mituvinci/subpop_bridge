#!/usr/bin/env bash
# Three-stage BRIDGE chain for ImageNetBG (seed 0)
# AG dataset -- no group annotations, so NO Step 2 (with attr).
# Step 1: s20 merge (ERM* + 9 specialists, Optuna 50 trials)
...
#     --job-name=chain3_imagenetbg_s0 \
#     --output=logs/chain3_imagenetbg_s0_%j.out \
#     --error=logs/chain3_imagenetbg_s0_%j.err \
#     scripts/chain_3stage_imagenetbg.sh
