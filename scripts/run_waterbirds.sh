#!/usr/bin/env bash
# Full BRIDGE chain for Waterbirds: ERM* -> BRIDGE (attr + noattr)
#
# Usage:
#   SEED=0 sbatch --time=08:00:00 --partition=gpu_2day \
#     --mem=48G --gres=gpu:1 --cpus-per-task=8 \
#     --job-name=chain_waterbirds_s0 \
#     --output=logs/chain_waterbirds_s0_%j.out \
#     --error=logs/chain_waterbirds_s0_%j.err \
#     scripts/run_waterbirds.sh
