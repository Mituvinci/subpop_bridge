#!/usr/bin/env bash
# Three-stage BRIDGE chain for CivilComments (seed 0)
# Step 1: s20 merge (ERM* + 16 specialists, Optuna 50 trials)
...
#     --job-name=chain3_civilcomments_s0 \
#     --output=logs/chain3_civilcomments_s0_%j.out \
#     --error=logs/chain3_civilcomments_s0_%j.err \
#     scripts/chain_3stage_civilcomments.sh
...
ERM_CKPT=$(ls -t results/s19_subpop_bert_star/civilcomments/erm/*_s${SEED}/model_best_wga.pt results/s19_subpop_bert_star/civilcomments/erm/*_s${SEED}/model.pt 2>/dev/null | head -1 || true)
...
echo "[chain] All 16 specialist dirs verified."
