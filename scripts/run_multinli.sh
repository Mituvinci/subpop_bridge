#!/usr/bin/env bash
# Three-stage BRIDGE chain for MultiNLI (seed 0)
# Step 1: s20 merge (ERM* + 6 specialists, Optuna 50 trials)
# Step 2: s26f WITH attribute annotations (--val-balance attribute)
# Step 3: s26f WITHOUT attribute annotations (--no-group-roles)
#
# Usage:
#   sbatch --time=18:00:00 --partition=gpu_2day \
#     --mem=48G --gres=gpu:1 --cpus-per-task=4 \
#     --job-name=chain3_multinli_s0 \
#     --output=logs/chain3_multinli_s0_%j.out \
#     --error=logs/chain3_multinli_s0_%j.err \
#     scripts/chain_3stage_multinli.sh
...
ERM_CKPT=$(ls -t results/s19_subpop_bert_star/multinli/erm/*_s${SEED}/model_best_wga.pt results/s19_subpop_bert_star/multinli/erm/*_s${SEED}/model.pt 2>/dev/null | head -1 || true)
S19_ROOT=results/s19_subpop_vit
...
# Verify inputs exist
if [ ! -f "$ERM_CKPT" ]; then
    echo "ERROR: ERM* checkpoint not found: $ERM_CKPT" >&2
    exit 1
fi
for g in 0 1 2 3 4 5; do
    SPEC="$S19_ROOT/$DATASET/bridge_specialist/group_${g}"
    if [ ! -d "$SPEC" ]; then
        echo "ERROR: Specialist group_${g} not found: $SPEC" >&2
        exit 1
    fi
done
echo "[chain] All 6 specialist dirs verified."
...
if [ -z "$MERGE_DIR" ] || [ ! -f "$MERGED_CKPT" ]; then
    echo "ERROR: merged_model.pt not found after s20" >&2
    exit 1
fi
